...
function _createBooks() {
    ...
    if (!Array.isArray(gBooks) || !gBooks.length) {
        console.error("Error: gBooks is undefined or empty!");
        return
    }

    if (!books || !books.length) {
        books = gBooks.map(book => _createBookFromData(book))
...
function _createBookFromData(book) {
    return {
        ...
        description: book.description || utilService.makeLorem(50),
        pageCount: book.pageCount || utilService.getRandomInt(1, 700),
        categories: book.categories || ["General"],
        thumbnail: book.thumbnail || "https://via.placeholder.com/150", // Default image
        language: book.language || "en",
        listPrice: {
            amount: (book.listPrice && book.listPrice.amount) ? book.listPrice.amount : utilService.getRandomInt(10, 30),
